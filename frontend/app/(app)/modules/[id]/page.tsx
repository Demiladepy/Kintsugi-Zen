...
export default function ModulePage({ params }: ModulePageProps) {
  const module = mockModules.find((item) => item.id === params.id) ?? mockModules[0];

  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_280px]">
      <section className="space-y-5 rounded-2xl border border-border bg-card p-6">
        <p className="text-sm text-muted-foreground">Library / {module.subject} / {module.title}</p>
        <div className="aspect-[16/7] rounded-xl bg-gradient-to-br from-primary/15 via-accent-violet/10 to-muted" />
        <header>
          <h1 className="text-h1">{module.title}</h1>
      ...
      <aside className="sticky top-24 h-fit rounded-2xl border border-border bg-card p-4">
        ...
        <p className="text-sm text-muted-foreground">👁 {module.reads} · ✨ {module.adaptations} · 💛 {module.appreciations}</p>
